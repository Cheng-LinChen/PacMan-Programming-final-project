...
extern bool ghostblink = 0;
extern int ghost_detain[4] = {0};
...
static void init(void) {
	...
	// Create map from .txt file and design your own map !!
	 basic_map = create_map("Assets/mapp.txt");
	if (!basic_map) {
...
static void status_update(void) {
	for (int i = 0; i < GHOST_NUM; i++) {
		...
		if((!p_st) &&!cheat_mode && RecAreaOverlap(getDrawArea(pman->objData, GAME_TICK_CD), getDrawArea(ghosts[i]->objData, GAME_TICK_CD)))
		{
		...
		if ((p_st) && !cheat_mode && RecAreaOverlap(getDrawArea(pman->objData, GAME_TICK_CD), getDrawArea(ghosts[i]->objData, GAME_TICK_CD)))
		{
			game_log("collide with ghost,power mode\n");
...
static void update(void) {
	...
	if (((basic_map->beansCount) - (basic_map->beansNum)) > eaten_bean_num || (basic_map->beansNum)==0) {
...
static void draw(void) {
	...
	al_draw_textf(
		...
		"Counting: %d",(basic_map->beansCount - basic_map->beansNum)
	);
	int t;
	if (eaten_bean_num < basic_map->beansCount) {
		t = eaten_bean_num;
	}
	else {
		t = basic_map->beansCount;
	}
	al_draw_textf(
		menuFont,
		al_map_rgb(255, 255, 0),
...
static void vinit() {
	gameTitle = load_bitmap("Assets/vic.png");
	tra = button_create(45, 610, 130,130, "Assets/lap.png", "Assets/laph.png");
	gameTitleW = al_get_bitmap_width(gameTitle);
	gameTitleH = al_get_bitmap_height(gameTitle);
}
static void von_mouse_move(int a, int mouse_x, int mouse_y, int f) {

	tra.hovered = pnt_in_rect(mouse_x, mouse_y, tra.body);

}
//
static void von_mouse_down() {
	game_log("von_mouse_down\n");
	if (tra.hovered && on_mouse_down)
		game_change_scene(scene_menu_create());
}
static void vdestroy() {
	...
	al_destroy_bitmap(tra.default_img);
	al_destroy_bitmap(tra.hovered_img);


}
static void von_key_down(int keycode) {
...
Scene scene_victory_create(void) {
	...
	scene.name = "Victory";
	scene.draw = &vdraw;
	scene.initialize = &vinit;
	scene.destroy = &vdestroy;
	scene.on_key_down = &von_key_down;
	scene.on_mouse_move = &von_mouse_move;
	scene.on_mouse_down = &von_mouse_down;
