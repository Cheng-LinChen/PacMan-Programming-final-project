...
extern bool ghostblink;
extern int p_st;
extern int ghost_detain[4];
extern int ghost_det_time=25;//5-50;
...
void ghost_draw(Ghost* ghost) {
	...
	*/
	...
	if (ghost->status == FLEE && (!ghostblink)) {
		if (al_get_timer_count(game_tick_timer) % 2) {
	...
	else if (ghost->status == FLEE && (ghostblink)) {
		if (((al_get_timer_count(game_tick_timer) % 16)/4) == 0) {
		...
		else if (((al_get_timer_count(game_tick_timer) %16)/4) ==1) {
			al_draw_scaled_bitmap(ghost->flee_sprite, 16, 0,
				16, 16,
				drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
				draw_region, draw_region, 0
		...
		else if (((al_get_timer_count(game_tick_timer) % 16)/4) == 2) {
	...
	else if (ghost->status == GO_IN && (ghost_detain[ghost->typeFlag] == 1)) {//(ghost->status == GO_IN)
...
void ghost_move_script_GO_IN(Ghost* ghost, Map* M) {
	...
	if (M->map[ghost->objData.Coord.y][ghost->objData.Coord.x] == 'B') {		
		ghost->speed = 2;
		if (ghost_detain[ghost->typeFlag] > 0) {
			ghost_detain[(ghost->typeFlag)]++;
			if (ghost_detain[ghost->typeFlag] > ghost_det_time) {
				game_log("ghost go out\n");
				ghost_detain[ghost->typeFlag] = 0;
				ghost->status = FLEE;
				ghost->speed = 1;
			}
		}
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	if (((pacman->objData.Coord.x) ^ 2 + (pacman->objData.Coord.y) ^ 2) -
		((ghost->objData.Coord.x) ^ 2 + (ghost->objData.Coord.y) ^ 2) < 100) {
		static Directions proba[4]; // possible movement
		int cnt = 0;
		int st = 0;
		for (Directions i = 1; i <= 4; i++) {
			if (ghost_movable(ghost, M, i, is_room_block) && (ghost->objData.preMove != 5 - i) && (i != shortestDirection)) {
				proba[cnt++] = i;
			}
		}
		if (cnt > 0) {
			ghost_NextMove(ghost, proba[(int)(generateRandomFloat() * cnt)]);
			st = 1;
		}
		if (st == 0 && cnt == 0 && (ghost_movable(ghost, M, 5 - ghost->objData.preMove, is_room_block))) {
	...
	if (((ghost->objData.Coord.x) ^ 2 + (ghost->objData.Coord.y) ^ 2) -
		((pacman->objData.Coord.x) ^ 2 + (pacman->objData.Coord.y) ^ 2) < 100) {
		static Directions proba[4]; // possible movement
		int cnt = 0;
		int st = 0;
		for (Directions i = 1; i <= 4; i++) {
		...
		int st = 0;
		for (Directions i = 1; i <= 4; i++) {
			if (ghost_movable(ghost, M, i, is_room_block) && (ghost->objData.preMove != 5 - i) ) {
				proba[cnt++] = i;
			}
		}
		if (cnt > 0) {
