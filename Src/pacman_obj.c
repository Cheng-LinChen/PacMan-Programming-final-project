...
extern ALLEGRO_TIMER* death_anim_counter;
...
void pacman_destory(Pacman* pman) {
	/*
		...
		free(pman);
	*/
	al_destroy_bitmap(pman->death_anim_counter);
	al_destroy_timer(pman->move_sprite);
	...
	*/
	...
	if (game_over) {
		...
		*/
		...
		al_draw_scaled_bitmap(pman->die_sprite,16*(int)(al_get_timer_count(pman->death_anim_counter)/10)
			, 0,
			16, 16,
			...
			{   
				...
				case RIGHT:
					if ((al_get_timer_count(game_tick_timer) % 32)/16) {
